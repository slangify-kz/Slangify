(()=>{
  ...
  function meaning(){shell(`${window.SlangOwnerProgress?.known(current.w)?'<p class="known-badge">Learned ✓ · marked as known in your profile</p>':''}<div class="studio-meaning"><p>${esc(current.m)}</p><p lang="kk">${esc(current.k)}</p></div><p class="studio-example">${esc(current.e)}</p>${current.note?`<p class="studio-owner-note">${esc(current.note)}</p>`:''}<div class="learning-tools"><button data-studio="speak">Listen to pronunciation</button><button class="primary" data-studio="learn">Start learning →</button></div><p class="studio-note">Learn through recall, a sentence gap and a short memory check. Your saved words stay in this browser.</p>`)}
  ...
  function ownerVideo(){
    ...
    Promise.resolve(window.SlangContent?.ready).then(()=>{
      if(request!==videoRequest||!dialog.open)return;
      ...
      const line=at<0?esc(caption):esc(caption.slice(0,at))+'<mark>'+esc(caption.slice(at,at+word.length))+'</mark>'+esc(caption.slice(at+word.length));
      dialog.querySelector('#studio-body').innerHTML=`<div class="studio-clip-heading"><h3>${esc(clip.title)}</h3><span>1 clip</span></div><video class="studio-native-video" controls playsinline preload="metadata" controlslist="nodownload noremoteplayback" disableremoteplayback src="${esc(clip.src)}" aria-label="Video example for ${esc(word)}">${clip.captions?'<track kind="captions" src="'+esc(clip.captions)+'" srclang="en" label="English" default>':''}</video><p class="studio-caption">${line}</p><div class="learning-tools"><button data-video-replay>Replay clip ↻</button><button data-video-speed aria-pressed="false">Slow · 0.75×</button><button data-studio="speak">Pronunciation</button></div><p class="studio-credit">${esc(clip.credit)}</p><p class="studio-note" id="video-status" role="status"></p>`;
      dialog.querySelector('video').addEventListener('error',()=>{if(request===videoRequest){dialog.querySelector('#video-status').textContent='This clip could not load. Please try again later.';dialog.querySelector('[data-video-replay]').disabled=true}});
    });
  }
  ...
  function library(){const collection={...saved};for(const w of window.SlangOwnerProgress?.collection()||[])collection[normalize(w.w)]={...w,...saved[normalize(w.w)],manual:true};lastFocus=document.activeElement;current={w:'Saved words',p:'YOUR COLLECTION'};shell(`<p class="studio-note">Choose a word to recall. Reviews due today appear first.</p><label for="saved-find">Find a saved word</label><input id="saved-find" class="studio-answer" type="search" placeholder="Search English or қазақша"><p id="saved-count" class="studio-note" role="status"></p><div class="studio-list">${Object.values(collection).sort((a,b)=>(a.due||0)-(b.due||0)).map(w=>`<button data-saved="${esc(normalize(w.w))}"><strong>${esc(w.w)}</strong><span>${w.manual?'Learned ✓':(w.due||0)<=Date.now()?'Review today':'Review tomorrow'}</span></button>`).join('')||'<p>No saved words yet. Open a word and choose Save word.</p>'}</div>`);dialog.querySelector('.studio-tabs').remove();const filterSaved=()=>{const q=normalize(dialog.querySelector('#saved-find').value);let count=0;dialog.querySelectorAll('[data-saved]').forEach(b=>{const w=collection[b.dataset.saved];b.hidden=!normalize([w.w,w.m,w.k].join(' ')).includes(q);if(!b.hidden)count++});dialog.querySelector('#saved-count').textContent=count+' words'};dialog.querySelector('#saved-find').oninput=filterSaved;filterSaved();dialog.querySelectorAll('[data-saved]').forEach(b=>b.onclick=()=>show(collection[b.dataset.saved]));if(!dialog.open)dialog.showModal()}
