(()=>{
'use strict';
const DAILY_KEY='slangify.daily-phrase.v4';
...
function uniquePool(){
 ...
 for(const item of source){const key=norm(item.w);if(!key||seen.has(key)||!item.m)continue;seen.add(key);out.push(item)}
 return out;
}
function blankExample(item){
 ...
 return rx.test(text)?text.replace(rx,(m,prefix)=>prefix+'_____'):text;
}
function distractors(item,pool,seed){
 const candidates=pool.filter(other=>norm(other.w)!==norm(item.w)&&norm(other.m)!==norm(item.m)&&norm(other.k)!==norm(item.k));
 return shuffled(candidates,seed).slice(0,3).map(x=>x.w);
}
...
function setup(){
 ...
 function render(){
  const item=pool[index],wrong=distractors(item,pool,baseSeed^seedFrom(item.w)^questionNo),options=shuffled([item.w,...wrong],baseSeed^seedFrom(item.w+'|options|'+questionNo));
  box.replaceChildren();
  ...
  const tag=document.createElement('span');tag.textContent=`QUESTION ${questionNo} · 4 OPTIONS · 1 CORRECT ANSWER`;top.append(heading,tag);
  const situation=document.createElement('p');situation.className='daily-situation';situation.textContent='Choose the one expression that matches this exact meaning.';
  const meaning=document.createElement('p');meaning.className='daily-gap';meaning.textContent=item.m+' · '+item.k;
  const example=blankExample(item);if(example){const ex=document.createElement('p');ex.className='daily-situation';ex.textContent=example;situation.after(ex)}
  const group=document.createElement('div');group.className='daily-options';group.setAttribute('role','group');group.setAttribute('aria-label','Choose the matching expression');
  ...
  function choose(answer,persist=true){
   ...
   feedback.textContent=(correct?'You got it. ':'The only correct answer is '+item.w+'. ')+item.m+' · '+item.k;
  ...
  actions.append(learn,next);box.append(top,situation,meaning,group,feedback,actions);
