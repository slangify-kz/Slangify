...
test('demo scores are visible but excluded from research aggregation and CSV',()=>{
  const d=profile(77);complete(d,'pre',12);lessons(d);complete(d,'post',20,1000010000);complete(d,'delayed',28,S.due(d));d.demo=true;d.demoScores={pre:40,post:68,delayed:92};
  const clean=S.cleanProfile(d);assert.equal(S.score(clean,'post').percent,68);assert.equal(S.score(clean,'delayed').percent,92);assert.equal(S.groups([clean]).find(g=>g.group==='context').n,0);assert.equal(S.csv([clean]).split('\r\n').length,1);
});
