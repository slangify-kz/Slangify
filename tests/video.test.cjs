...
test('video uses a local native player with replay and slower playback',async()=>{
 const a=setup([clip]);a.window.SlangStudio.open({w:'okay',m:'All right.',k:'Жарайды.',e:'Okay.'});a.click({studio:'video'});await new Promise(resolve=>setImmediate(resolve));
...
test('empty clip state is honest and pronunciation remains native',async()=>{
 const a=setup();a.window.SlangStudio.open({w:'lowkey',m:'A little.',k:'Аздап.',e:'I lowkey like it.'});a.click({studio:'video'});await new Promise(resolve=>setImmediate(resolve));
...
test('late content cannot reopen a dismissed player',async()=>{
 const a=setup([clip]);a.window.SlangStudio.open({w:'okay'});a.click({studio:'video'});a.dialog.close();await new Promise(resolve=>setImmediate(resolve));assert.equal(a.dialog.html,'');
});
