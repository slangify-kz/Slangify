...
function ensureStudyCore(){
 ...
 corePromise=(async()=>{if(!window.SlangCourse)await load('course-data.js?v=20260919');await load('study-core.js?v=20260925e');return window.SlangStudy})().catch(()=>null);
 return corePromise;
}
...
function addPanelStyles(){
 ...
 .owner-group{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-bottom:18px}.owner-group-card{padding:14px;border:1px solid var(--line,#d9dfda);border-radius:12px}.owner-group-card b{display:block;margin-bottom:4px}.owner-group-card span{font-size:12px;color:var(--muted,#657069)}
 .owner-demo{border:1px solid var(--line,#d9dfda);border-radius:14px;background:var(--surface-alt,#f0f1ec);padding:16px;margin:12px 0 24px}.owner-demo h3{margin:0 0 6px;font-size:18px}.owner-demo p{margin:0 0 12px;color:var(--muted,#657069);font-size:12px}.owner-demo .owner-group{margin:0}.owner-demo .owner-group-card{background:var(--surface,#fff)}
...
function renderPanel(dialog){
 ...
 if(study?.groups){const groups=document.createElement('div');groups.className='owner-group';for(const g of study.groups(profiles)){const card=document.createElement('div');card.className='owner-group-card';const b=document.createElement('b');b.textContent=(g.group==='context'?'Group A · Context':g.group==='classic'?'Group B · Standard':'Group C · Mixed')+' · n='+g.n;const s=document.createElement('span');s.textContent=`Paired ${g.paired} · Pre ${g.pre==null?'—':Math.round(g.pre)+'%'} · Post ${g.post==null?'—':Math.round(g.post)+'%'} · Gain ${g.gain==null?'—':(g.gain>=0?'+':'')+Math.round(g.gain)+' pp'} · 72h ${g.delayed==null?'—':Math.round(g.delayed)+'%'} · Active ${g.activeHours==null?'—':g.activeHours.toFixed(1)+' h'}`;card.append(b,s);groups.appendChild(card)}body.appendChild(groups)}
 const demo=document.createElement('section');demo.className='owner-demo';
 const demoTitle=document.createElement('h3');demoTitle.textContent='DEMO · example statistics for 3 groups';
 const demoNote=document.createElement('p');demoNote.textContent='Made-up values to preview the layout. They are never added to real results or exports.';
 const demoCards=document.createElement('div');demoCards.className='owner-group';
 const examples=[['Group A · Context','45%','82%','75%','4 h 10 min'],['Group B · Standard','45%','66%','60%','3 h 25 min'],['Group C · Mixed','45%','77%','70%','4 h 35 min']];
 for(const [name,pre,post,later,hours] of examples){const card=document.createElement('div');card.className='owner-group-card';const heading=document.createElement('b');heading.textContent=name;const info=document.createElement('span');info.textContent='Pre-test '+pre+' · Post-test '+post+' · 72h test '+later+' · Active '+hours;card.append(heading,info);demoCards.appendChild(card)}
 demo.append(demoTitle,demoNote,demoCards);body.appendChild(demo);
